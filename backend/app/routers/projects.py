...
Cuts v3 project API.
  ...
  GET  /api/projects/{id}/cuts-v3     latest ingest_run + every cut_record

See cuts_v3.plan.md section 7. Ingest itself runs on the procrastinate `l2`
queue (``app.services.l3.ingest.l3_cuts_v3_ingest``) -- this router only
...
from app.services.l3 import cuts_v3_read as read
...
def _owned_project(project_id: str, user_id: str) -> None:
    from app.services.l3.cuts_v3_read import _pg_conn
...
def kick_ingest(project_id: str, user_id: str = Depends(get_current_user_id)):
    """Enqueue (or re-enqueue) the cuts-v3 ingest for this project. Returns
    immediately; poll GET /{project_id}/cuts-v3 for status."""
...
@router.get("/{project_id}/cuts-v3")
def get_cuts_v3(project_id: str, user_id: str = Depends(get_current_user_id)):
    """All cut_records + project summary + ingest status. Pure DB read --
    zero model calls."""
    result = read.load_cuts_v3(project_id, user_id)
    if result is None:
        raise HTTPException(status_code=404, detail="project not found")
    return result
