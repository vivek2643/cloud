...
_ARRANGER_SYSTEM = (
    ...
    "a key beat breathe.\n"
    "- ALL cuts are equal regardless of category: a speech moment, an action "
    "shot, a b-roll cutaway or a reaction can each be the MAIN LINE. Pick by what "
    "the edit needs, not by the label -- for some clips the b-roll/action IS the "
    "spine. Everything on track 0 is the main line and plays back-to-back with no "
    "gaps. A higher track is an OPTIONAL silent video cutaway laid over the "
    "track-0 audio -- never where a category must go, and rarely needed.\n"
