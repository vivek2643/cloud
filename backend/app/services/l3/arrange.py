...
_ARRANGER_SYSTEM = (
    ...
    "  - 'dup:tgN' means this moment is the SAME spoken beat as others in group "
    "tgN (a retake or another camera angle); 'retry' marks an abandoned take. No "
    "take is pre-picked -- choose the member that reads/looks best by its text "
    "and score. Don't repeat the same line on the main line, but you MAY place "
    "another member as a silent reaction/cutaway over that line's audio (e.g. the "
    "listener while the speaker talks). A SAME-BEAT GROUPS section lists each "
    "group's members.\n\n"
...
_CRITIQUE_PROMPT = (
    ...
    "Check, in order:\n"
    "- Redundancy: any two cuts that say the same thing? Is the same line (a "
    "dup-group) repeated on the main line? Keep one read on the main line (a "
    "second member is fine only as a silent reaction/cutaway over that audio).\n"
