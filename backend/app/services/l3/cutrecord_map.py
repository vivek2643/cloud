...
def _file_is_musical(file_id: str) -> bool:
    ...
    brain_material_truth.plan.md Part 1: this is a PER-FILE flag -- a
    precondition speech_words_in_span checks before also consulting
    per-span evidence (_speaker_turns), never the whole answer on its own.
    A file with one music sting and an hour of narration reads True here
    for every span in it; that used to be the end of the question, which is
    exactly how a founder's own narration over his slides got muted as
    "music". Kept, not deleted -- it still correctly catches the case it
    was built for (transcribed song lyrics with no speaker attribution)."""
    try:
        with _pg_conn() as conn:
            row = conn.execute(
...
@lru_cache(maxsize=512)
def _speaker_turns(file_id: str) -> Tuple[Tuple[int, int], ...]:
    """This file's dialogue_segments SENTENCE-level turns that carry a
    speaker attribution -- (start_ms, end_ms) per turn, sorted by nothing
    in particular (overlap is a scan, not a search). The per-SPAN evidence
    speech_words_in_span consults before trusting _file_is_musical's
    per-file flag (brain_material_truth.plan.md Part 1). Mirrors
    footage_map._sentences_for_file's own query shape -- duplicated, not
    cross-imported, matching this module's established convention (see
    _words_for_file's own docstring). () on any read failure, missing row,
    or a turn with no speaker id -- never a hard failure, never a guess."""
    try:
        with _pg_conn() as conn:
            row = conn.execute(
        ...
        logger.exception("_speaker_turns: load failed for %s", file_id)
        return ()
    if not row or not row[0]:
        return ()
    ...
    for s in doc.get("sentence") or []:
        if not s.get("speaker"):
            continue
        try:
            a, b = int(s.get("src_in_ms", 0) or 0), int(s.get("src_out_ms", 0) or 0)
...
def speech_words_in_span(file_id: str, in_ms: int, out_ms: int) -> Tuple[int, bool, bool]:
    ...
    distinguishes a musical/ambient bed from genuine spoken narration.

    brain_material_truth.plan.md Part 1: is_musical is a property of THIS
    SPAN, not of the file. The file's own audio-type flag
    (_file_is_musical) is a precondition -- only when the FILE is musical
    do we even look for a reason to override it -- and a speaker-attributed
    dialogue_segments turn overlapping this exact span (_speaker_turns) is
    that reason: real, transcribed, spoken narration, not a lyric a
    diarizer happened to hear as speech. Known hole, not closed here: a
    diarizer that assigns a speaker id to a singer would read a genuine
    music file as "not musical" for that span. Closing that needs a
    time-resolved music classifier (audio_features.sections, NULL today),
    new computation out of scope for this plan. The accepted trade: a
    silenced narration is worse than a few audible seconds of a song the
    brain can see (SND:talk) and mute deliberately. Fail-open: any read
    failure -> (0, False, False), never a hard failure."""
    try:
        words = [
    ...
    if not words:
        return 0, False, False
    is_musical = _file_is_musical(file_id)
    if is_musical and any(a < out_ms and b > in_ms for a, b in _speaker_turns(file_id)):
        is_musical = False
    return len(words), True, is_musical
...
def _to_cut_dict(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        # sharpened scene specific (Pass B), when the background enrichment
        # has reached it -- {} until then (a normal, common state; cuts are
        # fully usable without it). {"specific", "label"}.
        "scene_specifics": row.get("scene_specifics") or {},
