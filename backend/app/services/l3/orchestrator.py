...
_PAGED_SYSTEM = (
    ...
    "Index notation: '<clip8>:<m##> channel.subject speaker [muted] .score [in-out play] \"gist\" · "
    "nrg:levels · run:rN · dup:tgN'. The play length (e.g. 6.4s) is what the cut "
    "runs after dead-air trimming -- use it to pace and hit a target length. "
    "'muted' on a done/shown cut = its source audio plays SILENT by default "
    "(still shows the picture): 'muted(talk)' a stray half-sentence, 'muted(sound)' "
    "uncontrolled/off-mic sound OR the action's own sound. Add \"audio\":\"keep\" to "
    "a pick to play that sound when it's the point (an action, laugh, applause, "
    "music), or \"audio\":\"mute\" to silence one. Refer to moments by FULL id. 'dup:tgN' = "
    ...
    "reaction/cutaway over that line's audio. 'run:rN' = a CONTINUOUS RUN: several "
    "moments of this clip that are ONE uninterrupted stretch of the source, back-"
    "to-back in time (listed in source order). Info, not a rule: by DEFAULT keep "
    "its members adjacent + in source order (they weld into one seamless shot) so "
    "continuous footage isn't fragmented by accident -- but you MAY break/intercut "
    "a run whenever the edit calls for it.\n\n"
    ...
    "an equal candidate for the main line -- never rank one channel over another; "
    ...
    "and rare. Cuts in the SAME clip whose [in-out] ranges overlap are SIMULTANEOUS "
    "(a line and the action/thing on screen at that instant): lay the silent "
    "video cut on a higher track over the line's audio to show what's talked about, "
    "or just pick whichever reads best. Consecutive main-line picks from the SAME "
    "clip whose source times touch/overlap are auto-welded into one continuous "
    "segment (no visible cut), so adjacent slices build a longer beat; pick "
    "non-adjacent slices to force a hard cut within a clip.\n\n"
