...
_LOOP_SYSTEM = (
    ...
    "GUESS FROM CONTEXT. The beat words, read in SOURCE ORDER, form one "
    "continuous narration of the footage. Use it. Wherever a cut's own "
    "description is thin, generic, or non-speech, actively INFER what it most "
    "likely shows and where its key moment falls from the words in the beats "
    "just before and after it -- what is being talked about predicts what is on "
    "screen, and emphasis ('watch this', 'look', 'and then--') flags where "
    "something matters even when you can't see it. Reason from that inference "
    "rather than treating the gap as empty or the cut as generic; place, time, "
    "and order cuts on the reading the surrounding words best support. Guess "
    "confidently when the context points somewhere; only ask when it genuinely "
    "underdetermines a choice that is the user's to make.\n\n"
