...
_SYSTEM = (
    ...
    "length.\n"
    "- ALL cuts are equal candidates, whatever their category. A speech moment, "
    "an action shot, a b-roll cutaway or a reaction can each be the MAIN LINE -- "
    "decide by what the edit needs, not by the label. For some clips the b-roll "
    "or action IS the spine (e.g. open the reel on the action shot). Put every "
    "cut that should play in sequence on track 0 (it plays back-to-back). Use a "
    "higher track ONLY if you deliberately want a silent video cutaway laid OVER "
    "the continuing track-0 audio, with a from_ms anchor -- it is an optional "
    "tool, never where a category 'has' to go.\n"
