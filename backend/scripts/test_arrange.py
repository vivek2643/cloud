#!/usr/bin/env python3
"""Tests for the timeline compiler's WELD pass (l3.arrange): adjacent same-clip
source-contiguous main-line cuts merge into one continuous segment, while real
intra-clip jumps and a cut's own keep_spans jump-cuts stay separate. Run:
    .venv/bin/python scripts/test_arrange.py
"""
...
from app.services.l3 import arrange  # noqa: E402
from app.services.l3.arrange import ResolvedCut, _segments_from_main  # noqa: E402
...
def test_contiguous_same_clip_welds():
    ...
    segs = _segments_from_main([_rc(FID_A, 0, 2000), _rc(FID_A, 2000, 4000)])
...
def test_near_touching_within_tolerance_welds():
    ...
    segs = _segments_from_main([_rc(FID_A, 0, 2000), _rc(FID_A, 2000 + gap, 4000)])
    assert len(segs) == 1, segs
...
def test_overlapping_same_clip_welds_to_union():
    ...
    segs = _segments_from_main([_rc(FID_A, 1000, 3000), _rc(FID_A, 2000, 4000)])
    assert len(segs) == 1, segs
...
def test_intra_clip_jump_stays_separate():
    ...
    segs = _segments_from_main([_rc(FID_A, 0, 2000), _rc(FID_A, 30000, 32000)])
...
def test_different_clips_never_weld():
    ...
    segs = _segments_from_main([_rc(FID_A, 0, 2000), _rc(FID_B, 0, 2000)])
    assert len(segs) == 2, segs
    print("ok  different clips never weld")


def test_keep_spans_jumpcut_survives():
    """A cut's own windup|payoff keep_spans are non-contiguous, so the internal
    jump-cut survives the weld (it is not silently stitched back together)."""
    cut = _rc(FID_A, 0, 8000, channel="done",
              keep_spans=[{"in_ms": 0, "out_ms": 1000}, {"in_ms": 6000, "out_ms": 8000}])
    segs = _segments_from_main([cut])
    assert len(segs) == 2, segs
...
def test_welded_axis_is_speech_if_either_side_is():
    ...
    segs = _segments_from_main([
        _rc(FID_A, 0, 2000, channel="shown"),
        _rc(FID_A, 2000, 4000, channel="said"),
    ])
    assert len(segs) == 1, segs
...
def test_seg_ids_unique_after_weld():
    segs = _segments_from_main([
        _rc(FID_A, 0, 2000), _rc(FID_A, 2000, 4000),   # weld -> 1
        _rc(FID_B, 0, 1000),                            # -> 1
        _rc(FID_A, 0, 1000),                            # -> 1
    ])
