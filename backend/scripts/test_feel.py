...
def test_pace_and_totals():
    ...
    tl = [
        _seg("a", "f1", 0, 2000, content="one two three four", ref="f1:c00"),
        _seg("b", "f1", 2000, 4000, content="a b c d e f g h i j", ref="f1:c01"),
    ]
    r = feel.simulate(tl)
...
def test_video_cut_has_no_pace():
    tl = [_seg("a", "f1", 0, 3000, content="", axis="any", ref="f1:c00")]
    r = feel.simulate(tl, meta_by_ref={"f1:c00": {"channel": "shown"}})
    c = r.cuts[0]
...
def test_same_speaker_jump_cut_risk():
    tl = [
        _seg("a", "f1", 0, 3000, content="hello there friend", ref="f1:c00"),
        _seg("b", "f1", 3000, 6000, content="and one more thing", ref="f1:c01"),
        _seg("c", "f2", 6000, 9000, content="different person now", ref="f2:c00"),
    ]
    meta = {"f1:c00": {"speaker_person": "P0"}, "f1:c01": {"speaker_person": "P0"},
            "f2:c00": {"speaker_person": "P1"}}
...
def test_join_seamless_same_clip_contiguous_no_descriptor_lookup_needed():
    tl = [
        _seg("a", "f1", 0, 10000, content="", axis="any", ref="f1:c00"),
        _seg("b", "f1", 10100, 15000, content="", axis="any", ref="f1:c01"),
    ]
    r = feel.simulate(tl)
...
def test_join_same_clip_noncontiguous_similar_frames_is_a_jump_cut():
    ...
    tl = [
        _seg("a", "f1", 0, 408000, content="", axis="any", ref="f1:c00"),
        _seg("b", "f1", 209000, 209500, content="", axis="any", ref="f1:c01"),
    ]
...
def test_join_same_clip_noncontiguous_different_frames_is_a_cut_no_false_jump():
    ...
    tl = [
        _seg("a", "f1", 0, 10000, content="", axis="any", ref="f1:c00"),
        _seg("b", "f1", 45000, 50000, content="", axis="any", ref="f1:c01"),
    ]
...
def test_join_cross_file_similar_frames_is_seamless_not_blocked_by_file_id():
    ...
    tl = [
        _seg("a", "f1", 0, 3000, content="", axis="any", ref="f1:c00"),
        _seg("b", "f2", 0, 3000, content="", axis="any", ref="f2:c00"),
    ]
...
def test_join_content_relative_scale_same_bits_different_verdict():
    ...
    def _three_seam_timeline(other_dist):
        tl = [
            _seg("a", "f1", 0, 1000, content="", axis="any", ref="f1:c00"),
            _seg("b", "f1", 20000, 21000, content="", axis="any", ref="f1:c01"),
            _seg("c", "f1", 40000, 41000, content="", axis="any", ref="f1:c02"),
            _seg("d", "f1", 60000, 61000, content="", axis="any", ref="f1:c03"),
        ]
...
def test_join_regression_multi_content_take_yields_zero_jump_cuts():
    ...
    tl = [
        _seg("a", "f1", 0, 1000, content="", axis="any", ref="f1:c00"),
        _seg("b", "f1", 14000, 14500, content="", axis="any", ref="f1:c01"),
        _seg("c", "f1", 209000, 209500, content="", axis="any", ref="f1:c02"),
        _seg("d", "f1", 408000, 408500, content="", axis="any", ref="f1:c03"),
    ]
    descriptors = _desc(f1=[
...
def test_join_regression_clip93_backward_seam_is_a_jump_cut():
    tl = [
        _seg("a", "f1", 0, 408000, content="", axis="any", ref="f1:c00"),
        _seg("b", "f1", 209000, 209500, content="", axis="any", ref="f1:c01"),
        _seg("c", "f1", 14000, 14500, content="", axis="any", ref="f1:c02"),
    ]
    descriptors = _desc(f1=[
